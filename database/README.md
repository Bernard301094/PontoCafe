...
5. Aplique `005_multi_face_templates.sql` junto da versão de backend que suporta múltiplos templates. Essa migração remove a unicidade de `colaborador_id` em `templates_faciais` e permite manter, para a mesma pessoa, aparências complementares como com/sem touca e com/sem óculos. **Não aplique a 005 isoladamente mantendo o backend antigo em produção**, pois a rota antiga de cadastro ainda depende da restrição única anterior.
6. Execute `npm run auth:bootstrap` uma única vez para criar o primeiro administrador.
