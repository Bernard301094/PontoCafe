...
7. **Antes de implantar o Worker 0.15.0**, aplique `007_ponto_operation_idempotency.sql`. Ela cria o diário transacional exactly-once para `REGISTRO_RAPIDO`, `INICIAR` e `FINALIZAR`, permitindo replay seguro e reconciliação online → offline quando uma resposta de rede é perdida. **Depois da migração 007, implante o Worker 0.15 antes de instalar o APK Android 0.15.**
