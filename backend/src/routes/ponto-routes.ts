  ...
  try {
    const pausa = await transaction(async (client) => {
      ...
      try {
        ...
        return { id, periodo, limiteSegundos, foraHorario, inicioEm: inserted.rows[0]?.inicio_em }
      } catch (error: any) {
...
pontoRoutes.post('/pausas/finalizar', async (c) => {
  ...
  try {
    const pausa = await transaction(async (client) => {
      ...
      if (!verification.rows[0]) throw new AppError('Verificação facial inválida ou expirada.', 401)
      const open = await client.query<{ id:string; limite_segundos:number }>(
        `select id,limite_segundos from pausas_cafe where colaborador_id=$1 and fim_em is null order by inicio_em desc limit 1 for update`,
        [body.data.colaboradorId],
      )
      ...
      return { id: open.rows[0].id, fimEm: row.fim_em, duracaoSegundos: row.duracao_segundos, limiteSegundos: open.rows[0].limite_segundos, excedeuLimite: row.duracao_segundos > open.rows[0].limite_segundos }
    })
    return c.json(pausa)
  } catch (error) {
    if (error instanceof AppError) return c.json({ erro: error.message }, error.status)
